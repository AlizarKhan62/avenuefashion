...
$about_short_desc = mysqli_real_escape_string($con, $_POST['about_short_desc']);

$about_desc = mysqli_real_escape_string($con, $_POST['about_desc']);

$update_about_us = "UPDATE about_us SET about_heading='$about_heading', about_short_desc='$about_short_desc', about_desc='$about_desc' WHERE about_id=1";

$run_about_us = mysqli_query($con,$update_about_us);

if($run_about_us){

echo "<script>alert('About Us Page Has Been Updated Successfully!')</script>";

echo "<script>window.open('index.php?dashboard','_self')</script>";

} else {

echo "<script>alert('Error updating About Us: " . mysqli_error($con) . "')</script>";
